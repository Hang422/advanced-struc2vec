...
def _find_orca_executable():
    # Get the actual project root (parent of advancedStruc2vec)
    ...
    possible_locations = [
        os.path.join(project_root, "orca"),  # Original location
        os.path.join(project_root, "libs", "orca"),  # Parent libs location
        os.path.join(project_root, "advancedStruc2vec", "orca"),  # In advancedStruc2vec dir
    ]
...
def graphlet_correlation(M: np.ndarray) -> np.ndarray:
    """
    自定义相关性矩阵：对零方差列Output全 0，避免 NaN / RuntimeWarning。
...
def compute_graphlet_dgcd(graph: nx.Graph,
    ...
    # 2) nodes列表（按升序，以匹配 GDV 文件里“0..n-1”）
    nodes = sorted(graph.nodes())
    n = len(nodes)
    assert node_gdv.shape[0] == n, f"GDV 行数 {node_gdv.shape[0]} ≠ 图nodes数 {n}"
    idx_map = {node: i for i, node in enumerate(nodes)}

    # 3) Computing每个nodes每层的 DGCM（相关性矩阵）
    z = node_gdv.shape[1]
...
def compute_graphlet_dgcd_simple(graph: nx.Graph,
    ...
    用两个nodes在该层的和的绝对差值作为该层的距离，累加得到最终
    的分层距离字典。
    """
    # 1) 确定 max_layer
    if max_layer is None:
    ...
    nodes = sorted(graph.nodes())
    n = len(nodes)
    assert node_gdv.shape[0] == n, "GDV 行数必须等于nodes数"
    idx = {v: i for i, v in enumerate(nodes)}
    z = node_gdv.shape[1]

    # 3) 为每个nodes、每层做 DGCM，然后把矩阵“预累加”成一个标量 sumC[ui][ℓ]
...
def compute_graphlet_dgcd_simple13(
    ...
    用两个nodes在该层的和的绝对差值作为该层的距离，累加得到 per-layer 距离。
    """
    # 1) 确定 max_layer
    if max_layer is None:
        G0 = graph.to_undirected()
    ...
    # 3) 对每个nodes、每层Computing “前13×13 子矩阵的元素和”
    sumC = [dict() for _ in range(n)]
    for ui, u in enumerate(nodes):
        visited = {u}
        q = deque([(u, 0)])
...
#     end = time.time()
#     print(fw"代码运行Time：{end - start:.6f} 秒")
...
#     end = time.time()
#     print(f"代码运行Time：{end - start:.6f} 秒")
