...
from GraphEmbedding.ge.alias import create_alias_table
from GraphEmbedding.ge.utils import partition_dict, preprocess_nxgraph
from GraphEmbedding.ge.walker import BiasedWalker


class Struc2Vec():
    def __init__(self, graph, walk_length=10, num_walks=100, workers=1, verbose=0, stay_prob=0.3, opt1_reduce_len=False,
                 opt2_reduce_sim_calc=False, opt3_num_layers=3, temp_path='./temp_struc2vec/', reuse=False, structural_dist_file: str = None):
        self.graph = graph
        ...
        self.structural_dist_file = structural_dist_file
    ...
    def _compute_structural_distance(self, max_num_layers, workers=1, verbose=0, ):
        if self.structural_dist_file is not None:
            if not os.path.exists(self.structural_dist_file):
                raise FileNotFoundError(f"结构距离文件未找到: {self.structural_dist_file}")
            print(f"📦 载入结构距离文件: {self.structural_dist_file}")
            return pd.read_pickle(self.structural_dist_file)
        ...
        for layer in layers_adj:
            ...
            for v, neighbors in neighbors.items():
                ...
                if sum_w > 0:
                    ...
                    # 防止除 0：若所有结构距离太大，退化为均匀分布
                    if e_list:
                        e_list = [1.0 / len(e_list)] * len(e_list)
                    else:
                        e_list = []  # 无邻居
